...
- **Admin portal**: log in, dashboard KPIs (including live MRR/ARR from real membership data),
  ...
  menu button and content takes the full screen, so it's usable on an actual phone too. On mobile,
  a back chevron appears next to that menu button on every tab except Dashboard, returning there
  in one tap (the sidebar itself is still reachable via the menu button, but this avoids needing to
  open it just to get back to the home tab). Every modal throughout the app (admin CRUD, Property
  Profile fields, invites, etc.) has an explicit ✕ close button, not just tap-the-backdrop.
